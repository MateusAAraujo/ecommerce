...
$app->get('/order/:idorder/pagseguro', function($idorder){
	...
	'cart'=>$cart->gerValues(),
	'products'=>$cart->getValues(),
	'phone'=>[
	    'areaCode'=>substr($order->getnrphone(), 0, 2),
	    'number'=>substr($order->getphone(), 2, strlen($order->getnrphone()))
	]
    ]);
});
...
$app->post('/register', function(){
	...
	User::setErrorRegister('Preecha o seu nome!');
	header('Location: /login');
	exit;
    endif;
    
    if(!isset($_POST['email']) || $_POST['email'] == ''):	
	User::setErrorRegister('Preecha o seu e-mail!');
	header('Location: /login');
	exit;
    endif;
    
    if(!isset($_POST['password']) || $_POST['password'] == ''):	
	User::setErrorRegister('Preecha o sua senha!');
	header('Location: /login');
	exit;
    endif;
    
    if(User::checkLoginExist($_POST['email']) === true):
	User::setErrorRegister('Este e-mail já está sendo utilizado por outro usuário');
	header('Location: /login');
	exit;
    endif;
...
// DADOS DO SEU CLIENTE
    ...
    $dadosboleto["endereco1"] = utf8_decode($order->getdesaddress()) . ' ' . $order->getdesdistrict1();
